...
public class OdometryCode extends Exponential_Methods {
    public void runOpMode() throws InterruptedException {
        super.runOpMode();
        waitForStart();
        File testCases = AppUtil.getInstance().getSettingsFile("opModeData.txt");
        try {
            ReadWriteFile.readFile(testCases);
            double pLin = Double.parseDouble(read.readLine());
            double pRot = Double.parseDouble(read.readLine());
            double Tolerance = Double.parseDouble(read.readLine());
            ...
            while (opModeIsActive()) {
                ...
                if(gamepad1.a&&time.seconds()>.5){
                    pLin = Double.parseDouble(read.readLine());
                    pRot = Double.parseDouble(read.readLine());
                    Tolerance = Double.parseDouble(read.readLine());
                    time.reset();
                }
                telemetry.addData("pLin", pLin);
                telemetry.addData("pRot", pRot);
            ...
            telemetry.addData("crash", e.toString());
            telemetry.update();
            wait(1000000);
        }
    }
